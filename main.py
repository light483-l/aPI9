...
class MainWindow(QMainWindow):
    ...
    def update_address_display(self):
        ...
        try:
            if self.show_postcode:
                geocoder_params = {
                    "apikey": self.map_key,
                    "geocode": self.current_address,
                    "format": "json",
                    "kind": "house"
                }
                ...
                address_details = feature["metaDataProperty"]["GeocoderMetaData"]["Address"]

                postcode = address_details.get("postal_code", "")
                if postcode:
                    display_text = f"{self.current_address} (индекс: {postcode})"
                else:
                    display_text = f"{self.current_address} (индекс не найден)"
            else:
            ...
            self.address_output.setPlainText(self.current_address)
